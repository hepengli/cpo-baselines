...
class MixedPdType(PdType):
    def __init__(self, action_spaces):
        ...
        size = dict()
        for ac_space in action_spaces:
            if isinstance(ac_space, spaces.Box):
                assert len(ac_space.shape) == 1
                size["Box"] = ac_space.shape[0]
            elif isinstance(ac_space, spaces.MultiBinary):
                size["MultiBinary"] = ac_space.n
            else:
                raise NotImplementedError
        self.size = size
    def pdclass(self):
        return MixedPd
    def param_shape(self):
        return [2*self.size["Box"]+self.size["MultiBinary"]]
    def sample_shape(self):
        return [sum(self.size.values())]
    def sample_dtype(self):
        return tf.float32
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        logits = _matching_fc(latent_vector, 'pi/logits', self.size["MultiBinary"], init_scale=init_scale, init_bias=init_bias)
        mean = _matching_fc(latent_vector, 'pi/mean', self.size["Box"], init_scale=init_scale, init_bias=init_bias)
        logstd = tf.get_variable(name='pi/logstd', shape=[1, self.size["Box"]], initializer=tf.zeros_initializer())
        pdparam = [logits, tf.concat([mean, mean * 0.0 + logstd], axis=1)]
        return self.pdfromflat(pdparam), mean
...
class MixedPd(Pd):
    def __init__(self, flat):
        self.BernoulliPd = BernoulliPd(flat[0])
        self.DiagGaussianPd = DiagGaussianPd(flat[1])
    def flatparam(self):
        raise NotImplementedError
        # return self.flat
    def mode(self):
        return tf.concat([tf.round(self.BernoulliPd.ps), self.DiagGaussianPd.mean], axis=1)
    def neglogp(self, x):
        Bern_x = x[:,:self.BernoulliPd.logits.shape[1]]
        Gaus_x = x[:,self.BernoulliPd.logits.shape[1]:]
        return self.BernoulliPd.neglogp(Bern_x) + self.DiagGaussianPd.neglogp(Gaus_x)
    def kl(self, other):
        assert isinstance(other, MixedPd)
        return self.BernoulliPd.kl(other.BernoulliPd) + self.DiagGaussianPd.kl(other.DiagGaussianPd)
    def entropy(self):
        return self.BernoulliPd.entropy() + self.DiagGaussianPd.entropy()
    def sample(self):
...
def make_pdtype(ac_space):
    ...
    if isinstance(ac_space, spaces.Box):
        ...
        return MixedPdType(ac_space.__dict__["spaces"])
    else:
        raise NotImplementedError
