...
class Model(object):
    def __init__(self, *, policy, ob_space, ac_space, ent_coef, name_scope, load_path, vf_stepsize, vf_iters, 
                qf_stepsize, qf_iters, cg_damping, cg_iters, max_kl, max_sf):
        self.sess = sess = U.get_session()

        self.OB = OB = observation_placeholder(ob_space)
        # CREATE OUR TWO Policies
        with tf.variable_scope(name_scope+"pi"):
            pi = policy(sess=sess, observ_placeholder=OB)
        with tf.variable_scope(name_scope+"oldpi"):
            oldpi = policy(sess=sess, observ_placeholder=OB)
        ...
        self.Q = Q = tf.placeholder(dtype=tf.float32, shape=[None, ac_space.shape[0]+1])
        ...
        qferr = tf.reduce_mean(tf.square(pi.qf - Q))

        # Get trainable variables
        all_var_list = get_trainable_variables(name_scope+"pi")
        var_list = get_pi_trainable_variables(name_scope+"pi")
        vf_var_list = get_vf_trainable_variables(name_scope+"pi")
        qf_var_list = get_qf_trainable_variables(name_scope+"pi")

        self.vfadam = MpiAdam(vf_var_list)
        self.qfadam = MpiAdam(qf_var_list)
        self.vf_stepsize = vf_stepsize
        self.vf_iters = vf_iters
        self.qf_stepsize = qf_stepsize
        self.qf_iters = qf_iters
        ...
        self.assign_old_eq_new = U.function([],[], updates=[tf.assign(oldv, newv)
            for (oldv, newv) in zipsame(get_variables(name_scope+"oldpi"), get_variables(name_scope+"pi"))])
        ...
        self.compute_qflossandgrad = U.function([OB, Q], U.flatgrad(qferr, qf_var_list))
...
class QModel(object):
    def __init__(self, env, Q_fn, stepsize=3e-4, iters=3, load_path=None):
        U.get_session()
        ...
        # CREATE THE PLACEHOLDERS
        self.Q = Q = tf.placeholder([None, 2])

        # Define the loss function of the value network
        loss = tf.reduce_mean(tf.square(qf.q - self.Q))

        # Get trainable variables
        var_list = [q for q in get_trainable_variables('q') if 'q' in q.name[len('q'):].split('/')]
        ...
        # Compute loss, safety, graident and fvp values
        self.compute_lossandgrad = U.function([qf.OB, qf.ac, Q], loss, U.flatgrad(loss, var_list))
    ...
    def train(self, obs, values):
        # update value network
        with self.timed("value"):
            for _ in range(self.iters):
                for (mbob, mbret) in dataset.iterbatches((obs, values),
                include_final_partial_batch=False, batch_size=64):
                    g = self.allmean(self.compute_lossandgrad(mbob, mbret))
                    self.adam.update(g, self.stepsize)
...
def get_qf_trainable_variables(scope):
    return [q for q in get_trainable_variables(scope) if 'qf' in q.name[len(scope):].split('/')]
